    ...
    public static class Program
    {
        ...
        private static void ThreadDemo()
        {
            var thread = new Thread(() =>
                Console.WriteLine($"Hello from another thread: {Thread.CurrentThread.ManagedThreadId}"))
            {
                IsBackground = true,
        ...
        private static void ThreadExceptionDemo(bool background)
        {
            var thread = new Thread(() =>
            {
                Thread.Sleep(1000);
                throw new Exception(
                    $"Exception from {((background ? "background" : "foreground"))} thread: " +
        ...
        private static void GlobalExceptionHandling()
        {
            AppDomain.CurrentDomain.UnhandledException += (_, args) =>
            {
                ...
                {
                    Console.WriteLine(
                        $"Exception! {args.ExceptionObject.GetType().Name}: {Thread.CurrentThread.ManagedThreadId}");
                    Thread.Sleep(1000);
                }
            };
            ThreadExceptionDemo(false);
